...
def count_symb(message):
    ...
    if message.content_type != "text":
        ...
        if response.status_code == 200:
            ...
            user_symbols_for_tts = cur.execute(f'''SELECT symbols_for_tts FROM users_data WHERE user_id = {user_id}'''
                                               ).fetchone()[0]
...
def count_sec(message):
    ...
    if message.content_type != "voice":
        ...
        sounds_for_sst_sec = cur.execute(f'''SELECT sounds_for_sst FROM users_data WHERE user_id = {user_id}'''
                                         ).fetchone()[0]
