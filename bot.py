...
def welcome(message):
    ...
    connection = sqlite3.connect('database.db')
    cur = connection.cursor()
    logging.info(f"Пользователь с id - {user_id} использовал команду /start")
    if not exist_user(user_id):
        # Если пользователя нет, добавляем его в базу данных
        sql_query = ("INSERT INTO users_data (user_id, user_name, user_role, tokens, symbols_for_tts, "
                     "task_for_tts, sounds_for_sst, task) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
        cur.execute(sql_query, (
            user_id, user_name, "user", MAX_TOKENS, MAX_SYMBOLS_FOR_USER, MAX_TOKENS_FOR_USER,
            MAX_SOUNDS_FOR_SST_FOR_USER,
            " "))
        connection.commit()
        logging.info(f"Новый пользователь с id - {user_id} добавлен в базу данных")
        bot.send_message(message.chat.id,
                         "Приветствую, пользователь!\nДля работы с GPT пришлите промт в виде текста "
...
def tts_func(message):
    ...
    connection = sqlite3.connect('database.db')
    cur = connection.cursor()
    logging.info(f"Пользователь с id - {user_id} использовал команду /tts")
    user_symbols_for_tts = 0
    try:
        user_symbols_for_tts = (
            count_symb(cur.execute(f'''SELECT symbols_for_tts FROM users_data WHERE user_id = {user_id}''').fetchone(
            )[0]))

    except:
        logging.warning(f"Пользователю с id - {user_id} не удалось получить его символы для tts")

    if exist_user(user_id):
        if not is_limit_users():
            if user_symbols_for_tts >= MAX_SYMBOLS_FOR_USER:
                bot.send_message(message.chat.id, text="У вас закончилось доступное кол-во символов")
                logging.info(f"У пользователя с id - {user_id} закончилось доступное кол-во символов для tts")
            ...
            logging.warning("Бот перегружен")
...
def stt_func(message):
    ...
    connection = sqlite3.connect('database.db')
    cur = connection.cursor()
    user_sounds_for_sst = 0
    logging.info(f"Пользователь с id - {user_id} использовал команду /stt")
    try:
        user_sounds_for_sst = (
            cur.execute(f'''SELECT sounds_for_sst FROM users_data WHERE user_id = {user_id}''').fetchone())[0]

    except:
        logging.warning(f"Пользователю с id - {user_id} не удалось получить его аудио блоки для stt")

    if exist_user(user_id):
        if not is_limit_users():
            if user_sounds_for_sst >= MAX_SOUNDS_FOR_SST_FOR_USER:
                bot.send_message(message.chat.id, text="У вас закончилось доступное кол-во звуковых сообщений")
            else:
...
def stt_func(message):
    ...
    connection = sqlite3.connect('database.db')
    cur = connection.cursor()
    logging.info(f"Пользователь с id - {user_id} использовал команду /logs")
    try:
        user_role = cur.execute(f'''SELECT user_role FROM users_data WHERE user_id = {user_id}''').fetchone()[0]
    except:
        logging.warning(f"Пользователю с id - {user_id} не удалось получить его должность")
...
def send_text(message):
    ...
    if not exist_user(user_id):
        if not is_limit_users():
            try:
                sql = ("INSERT INTO users_data (user_id, user_name, user_role, tokens, symbols_for_tts, task_for_tts, "
                       "sounds_for_sst, task) VALUES (?, ?, ?, ?, ?, ?, ?, ?);")
